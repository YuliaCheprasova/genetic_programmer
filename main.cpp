    ...
    if(switch_init == 0)
    {
        for(i = 0; i < n; i++)
        {
            ...
            while (success == 1)
            {
                ...
                {
                    random_const = rand() % 11;//тут узнать и поменять
...
// нужно спросить как преобразовать дерево в математическое выражение, еще вот сейчас я использую две прототипные функции,
// в которых написано одно и то же, но типы разные из-за типа констант, может можно по-другому сделать?
